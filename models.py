...
from config import (batchsize, max_p_len, glove_dim,
...
# ---------------------reading
# NOTE:  It's unclear if there are a total of 4 bilstms or 2 from the paper.
# The possibilities are listed below.
# - b1, b2, b3, b4 = high context, high question, low context, low question
# - b1, b2 = question high+low, contex high+lowt
# - b1, b2 = high question+context, low question+context
# the sentence in the paper is
#   >>> in the reading component, we use separate bilstm to form low level and
#   >>> high level concepts for C and Q
# We use the first option because
# - high and low need different bilstms as per the sentence
# - the representations of the para and q are of different dim.
...
with tf.variable_scope("Reading"):
    ...
    ques_low_h, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=f_read_q_low,
                                                    cell_bw=b_read_q_low,
                                                    inputs=ques_w_rep,
                                                    dtype=tf.float32,
    ...
    ques_high_h, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=f_read_q_high,
    ...
    para_low_h, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=f_read_p_low,
    ...
    para_high_h, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=f_read_p_high,
...
with tf.variable_scope("final_q_und"):
    ...
    final_q_und, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=f_uq,
                                                     cell_bw=b_uq,
                                                     inputs=inp,
                                                     dtype=tf.float32,
                                                     scope='final_q_und')
...
with tf.variable_scope("high_level_fusion"):
    ...
    ff_para, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=f_vc,
                                                 cell_bw=b_vc,
                                                 inputs=inp,
                                                 dtype=tf.float32,
                                                 scope='full_fused_para')
...
with tf.variable_scope("self_boosting_fusion"):
    ...
    final_para_rep, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=f_sb,
                                                        cell_bw=b_sb,
                                                        inputs=inp,
                                                        dtype=tf.float32,
                                                        scope='self_boosted')
...
with tf.variable_scope("span_start"):
    ...
    for i, uc in enumerate(tqdm(uc_s, desc='StartSpan')):
        ...
        attention_weight.append(s)
    start_prediction = tf.nn.softmax(tf.stack(attention_weight, axis=1))
